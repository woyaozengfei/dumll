        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="page-title-normal checkout-title">
              <h2><span>您的订单</span></h2>
            </div>
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(item,index) in orderDataFilter" v-bind:class="{'check':checkIndex==index}" @click="checkIndex=index;selectedOrderId=item.orderId">
                    <dl>
                      <dt>订单号：{{item.orderId}}</dt>
                      <dd class="address">金额：{{item.orderTotal}}</dd>
                      <dd class="tel">{{item.createDate}}</dd>
                    </dl>
                    <div class="addr-opration addr-del">
                      <a href="javascript:;" class="addr-del-btn" @click="openOrderConfirm(item.orderId)">
                        <i class="el-icon-info message"></i>
                      </a>
                    </div>
                  </li>
                </ul>
...
<script>
  ...
  export default{
      ...
      computed:{
        orderDataFilter(){
          return this.orderData.slice(0,this.limit);
        }
      },
      components:{
        NavHeader,
      ...
      methods:{
          ...
          expand(){
              if(this.limit==4){
                this.limit = this.orderData.length;
              }else{
                this.limit = 4;
              }
          },
          //打开删除弹窗
          openOrderConfirm(orderId){
            this.orderShow = true;

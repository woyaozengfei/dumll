<template>
    <div>
          ...
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(item,index) in addressListFilter" v-bind:class="{'check':checkIndex==index}" @click="checkIndex=index;selectedAddrId=item.addressId">
                    ...
                    <div class="edit-icon">
                      <a href="javascript:;" class="el-icon-edit icon-edit" @click="editAddressConfirm(item)">
                      </a>
                    </div>
              ...
              <div class="shipping-addr-more">
                <a class="addr-more-btn up-down-btn" href="javascript:;" @click="expand" v-bind:class="{'open':limit>4}">
                  more
                  <i class="i-up-down">
      ...
      <el-dialog title="编辑您的收货地址" :visible.sync="isMdShowEdit" center width="20%">
        ...
        <div slot="footer" >
          <a class="btn btn--m" href="javascript:;" @click="editAddress"> 保 存 </a>
          <a class="btn btn--m btn--red" href="javascript:;" @click="close"> 取 消 </a>
        </div>
      </el-dialog>

      <el-dialog title="新增收货地址" :visible.sync="isMdShowAdd" center width="20%">
        ...
        <div slot="footer" >
          <a class="btn btn--m" href="javascript:;" @click="addAddress"> 保 存 </a>
          <a class="btn btn--m btn--red" href="javascript:;" @click="close"> 取 消 </a>
        </div>
      </el-dialog>
  ...
  export default{
      ...
      methods:{
          ...
          editAddressConfirm(item){
            this.isMdShowEdit = true;
            this.editForm.userName = item.userName
            this.editForm.addressId = item.addressId;
            this.editForm.streetName = item.streetName;
            this.editForm.postCode = item.postCode;
            this.editForm.tel = item.tel;
            this.editForm.isDefault = item.isDefault;
          },
          // 打开新增弹窗
          addAddressConfirm(){
          ...
          // 发起编辑请求
          editAddress(){
            this.isMdShowEdit = false;
            axios.post("/users/editAddress",this.editForm).then((response)=>{
                let res = response.data;
                if(res.status=="0"){
                  this.init();
                  this.close()
          ...
          addAddress(){
            ...
            axios.post("/users/addAddress",this.addForm).then((response)=>{
                let res = response.data;
                if(res.status=="0"){
                  this.init();
                  this.close()
